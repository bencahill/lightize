...
class LDirectory {
	...
	public function getImages() {
		global $db;
		$sth = $db->prepare( "SELECT id,name,date,info,rating,edits,hiddenInDirectory FROM image WHERE directoryId=? ORDER BY date" );
	...
	private function createThumbnails( $images ) {
		foreach ( $images as $image_name ) {
			...
			$gimage->thumbnailImage( 180, 180, true );
			$gimage->writeImage( "$this->cacheDir/{$imagePath['filename']}_180.{$imagePath['extension']}" );
		}
	}
